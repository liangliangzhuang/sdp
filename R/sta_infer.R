...
sta_infer <- function(method = "MLE",
  ...
  if (method == "MLE") {
    mle_re <- optim(
      par = par, fn = MLE, process = process, type = type,
    ...
    if (type == "classical") {
      ...
      return(re = list(
        "summary" = round(rstan::summary(fit1)$summary[c(1, 2), c(4, 1, 8)], 4),
        "stan_re" = fit1
      )) # 给出stan原始的结果，根据这个结果做诊断和可视化
    } else if (type == "acc") {
      ...
      # Data setting
      process_data <- list(
        I = dim(delta_y)[1], J = dim(delta_y)[2],
      ...
      return(re = list(
        "summary" = round(rstan::summary(fit1)$summary[1:6, c(4, 1, 8)], 4),
        "stan_re" = fit1
      )) # 给出stan原始的结果，根据这个结果做诊断和可视化
    }
  }
